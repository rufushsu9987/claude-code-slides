...
const C = {
  ...
  success: '7FB08A', warning: 'A56A32', white: 'FFFFFF',
};
...
function cover() {
  ...
  title(s, '{{TITLE}}', { y: 1.8, h: 2.15, fontSize: 53, w: 11.2 });
  ...
  text(s, '{{TEMPLATE_NAME}} · 20 LAYOUT STARTERS', { x: .72, y: 6.86, w: 5.7, h: .2, fontFace: F.mono, fontSize: 8.6, color: C.muted, charSpacing: .8 });
...
function flow() {
  ...
  nodes.forEach((d) => {
    ...
    text(s, d.n, { x: d.x, y: d.y + .39, w: .52, h: .14, align: 'center', fontFace: F.mono, fontSize: 7.6, color: d.accent ? C.white : C.ink });
...
function annotatedVisual() {
  ...
  box(s, 5.0, 1.64, .9, 4.72, { fill: 'EFEBE4', line: C.border });
  for (let i = 0; i < 4; i += 1) box(s, 5.18, 1.95 + i * .82, .55, .55, { fill: C.surface, line: C.border });
  box(s, 10.5, 1.64, 1.85, 4.72, { fill: 'F2EEE7', line: C.border });
  ...
  [['1',5.55,1.92],['2',10.8,2.9],['3',11.75,5.35]].forEach(([n,x,y]) => {
    ...
    text(s, n, { x, y: y + .12, w: .42, h: .12, align: 'center', fontFace: F.mono, fontSize: 7.6, color: C.white, bold: true });
  });
...
function timeline() {
  ...
  [['01','Catalog','Name archetypes, geometry, and purpose.'],['02','Scaffold','Ship diverse starters in every format.'],['03','Validate','Audit sequence, density, and export.'],['04','Learn','Add patterns from real decks.']].forEach(([num, heading, detail], i) => {
    ...
    text(s, num, { x, y: 3.32, w: .64, h: .16, align: 'center', fontFace: F.mono, fontSize: 8.2, color: i === 1 ? C.white : C.muted });
