...
function visibleWordCount(section) {
...
async function inspectHtml(file) {
  ...
  return {
    ...
    errors,
    warnings,
    metrics: { slides: slideSections.length, notes: notesCount },
  };
}
  ...
  if (themeMatch && themeMatch[1] !== 'default') {
    ...
    if (!existsSync(siblingTheme)) {
      ...
      if (!/@theme\s+/i.test(css)) {
        warnings.push(issue('warning', 'marp-theme', 'theme.css has no /* @theme ... */ declaration.', siblingTheme));
      }
    }
  }

  return { format: 'marp', file, errors, warnings, metrics: { slides, headings } };
}
...
async function inspectPptx(file) {
  ...
  return { format: 'pptx', file, errors, warnings, metrics: { slides } };
}
