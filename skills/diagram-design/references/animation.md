...
Run the bundled self-check from this skill directory:

```bash
python3 scripts/self_check.py path/to/animated-diagram.html
```

It checks accessible SVG naming, single-file safety, and the core motion structure. For repository-level contribution gates, use the upstream project's full verifier suite.
