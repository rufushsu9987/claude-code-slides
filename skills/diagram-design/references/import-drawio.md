...
```bash
python3 scripts/drawio_extract.py <file> [--page N|NAME|all]
```
...
**Load the chosen type reference before drawing.** Its layout conventions win over anything the source did.
...
[`assets/example-import-drawio.html`](../assets/example-import-drawio.html) is a bundled worked example of this procedure (12 nodes, 8 edges, 2 container groups) at `format=html`, `size=doc-inline`, `detail=balanced`, `audience=mixed`.
