...
**The single source of truth for colors, typography, and tokens.** Every diagram draws from this — not from hex values inlined in other reference files. If you want to change the visual skin, change this file.

Default skin is a cool editorial palette — white-smoke paper, jet-black ink, atomic-tangerine accent, blue-slate muted. When used inside Claude Code Slides, prefer the selected deck theme's tokens; the upstream defaults remain available for standalone examples. Swap these values (or run [`onboarding.md`](onboarding.md)) and every new diagram inherits the new skin without touching any type-specific logic.
...
Every token is referred to by **semantic role**, not by its hex value. Type references and SKILL.md say `accent`, not `#f7591f`.
