...
Write the new tokens to `style-guide.md`. Suggest rebuilding one example to verify the new skin reads cleanly.
...
- **Brand has 6+ colors** and you can't identify a clear hierarchy: pick one as `accent`, demote the rest to `muted` variants or ignore them. The Diagram Design grammar only uses 5–7 roles.
...
Use the installed-skill location exposed by the current agent when available. Otherwise search the skill locations supported by that agent. When this skill is used inside a presentation workflow, the host's selected theme tokens are the preferred local brand source. Finally, check any path the user provides explicitly.
