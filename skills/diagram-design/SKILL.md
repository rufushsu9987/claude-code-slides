...
These mark low-quality diagrams of any type:
...
Type-specific anti-patterns live in the selected type reference.
...
- [ ] Ran the packaged `scripts/self_check.py` from this skill directory — clean? (Accessible-SVG contract, single-file safety, and motion basics.)
- [ ] If animated, does the complete static/no-JS frame work, does reduced motion hide/disable playback, and is the controller copied verbatim from `template-motion.html`? Run the bundled self-check and inspect print and static-query states before delivery.
...
2. If behavior is load-bearing, choose a semantic pattern; then load the matching type reference.
...
When the diagram will be embedded in a presentation, prefer `slide-16x9` sizing, keep the slide title, source note, and speaker notes editable outside the SVG, and save the diagram asset under the deck's `assets/` directory. For standalone use, always produce a single self-contained `.html` file:
